...
def get_sequences(dev_id, jump, seq_len):
    ...
    if dev_id != len(device_endpoints)-1:        
...
model.add(LSTM(units = neuron_input, input_shape = (X_train.shape[1], X_train.shape[2])))

model.add(Dropout(drop_rate)) 

model.add(Dense(units = neuron_hidden, activation='relu'))
...
fig, ax = plt.subplots()
ax2 = ax.twinx()   
ax2.plot(np.arange(epoch_num)+1,np.array(fitted_model.history['loss']), "r--", label = "Train loss")
ax.plot(np.arange(epoch_num)+1,np.array(fitted_model.history['accuracy']), "g--", label = "Train accuracy")
ax2.plot(np.arange(epoch_num)+1,np.array(fitted_model.history['val_loss']), "r-", label = "Validation loss")
ax.plot(np.arange(epoch_num)+1,np.array(fitted_model.history['val_accuracy']), "g-", label = "Validation accuracy")
plt.title("Accuracy and loss over increasing epochs")
ax.legend(bbox_to_anchor=(1.2,1), loc="upper left", borderaxespad=0)
ax2.legend(bbox_to_anchor=(1.2,0.6), loc="lower left", borderaxespad=0)
ax.set_ylabel('Train & Validation Accuracy')
ax2.set_ylabel('Train & Validation Loss')
...
dev_labels = np.argmax(y_test, axis=1)
pred_vs_gt = np.vstack([preds, dev_labels]).T
...
cm = confusion_matrix(pred_vs_gt[:,1], pred_vs_gt[:,0])
plt.title("Confusion Matrix")
cm = (cm.astype('float'))/ (cm.sum(axis=1)[:, np.newaxis]+0.0001)
heatmap(pd.DataFrame(cm), yticklabels = False, xticklabels = False)
plt.ylabel("Ground Truth for Device ID")
plt.xlabel("Predictions for Device ID")
plt.show()

    
end = time.time()
print("It took",end-start,"seconds to run the whole algorithm.")